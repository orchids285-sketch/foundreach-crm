...
import {
  assertSafeFormulaColumnExpression,
...
export const buildSqlColumnDefinition = (
  ...
  // asExpression is normally built server-side (search-vector derivation or the formula
  // transpiler), but it can technically reach here from user input (metadata API or app-sync
  // manifest). Validators reject corrupted expressions on every build path; this assert is the
  // last-resort guard at the DDL sink so nothing can break out of the GENERATED ALWAYS AS (...)
  // clause.
  if (isDefined(column.asExpression)) {
    if (column.type === 'tsvector') {
      assertSafeTsVectorExpression(column.asExpression);
    } else {
      assertSafeFormulaColumnExpression(column.asExpression);
    }
